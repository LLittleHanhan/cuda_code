...
template <int pack_size, typename FunctorT, typename R, typename... IN>
__device__ typename std::enable_if<HasApply2<FunctorT>::value == false || pack_size % 2 != 0, Packed<R, pack_size>>::type
ApplyPack(const FunctorT& functor, const Packed<IN, pack_size>... in) {
    Packed<R, pack_size> ret;
#pragma unroll
...
__global__ void __launch_bounds__(kBlockSize)
    ApplyGeneric(FactoryT factory, int64_t n_pack, Packed<R, pack_size>* pack_r, const Packed<IN, pack_size>*... pack_in, int64_t n_tail, R* tail_r, const IN*... tail_in) {
...
struct SimpleFactory {
    explicit SimpleFactory(FunctorT functor)
        : tpl(functor) {}
...
struct GenericLauncher {
    static cudaError_t Launch(FactoryT factory, int64_t n, R* r, const IN*... in, cudaStream_t stream) {
...
inline cudaError_t UnaryWithFactory(FactoryT factory, int64_t n, R* r, const A* a, cudaStream_t stream) {
...
inline cudaError_t BinaryWithFactory(FactoryT factory, int64_t n, R* r, const A* a, const B* b, cudaStream_t stream) {
...
inline cudaError_t Binary(FunctorT functor, int64_t n, R* r, const A* a, const B* b, cudaStream_t stream) {
...
inline cudaError_t TernaryWithFactory(FactoryT factory, int64_t n, R* r, const A* a, const B* b, const C* c, cudaStream_t stream) {
...
inline cudaError_t Ternary(FunctorT functor, int64_t n, R* r, const A* a, const B* b, const C* c, cudaStream_t stream) {
