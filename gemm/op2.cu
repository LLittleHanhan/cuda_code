...
__global__ void sgemm(float* da, float* db, float* dc, int M, int K, int N) {
    ...
    int row_float4_nums_da = BK / 4;
    int row_float4_nums_db = BN / 4;
    int row_float4_da = tid / row_float4_nums_da;
    int col_float4_da = tid % row_float4_nums_da * 4;
    int row_float4_db = tid / row_float4_nums_db;
    int col_float4_db = tid % row_float4_nums_db * 4;
    int row_stride_da = thread_per_block / row_float4_nums_da;
    int row_stride_db = thread_per_block / row_float4_nums_db;
// da
// #pragma unroll
...
//     }
    ...
    FETCH_FLOAT4(&da_reg[0]) = FETCH_FLOAT4(block_start_address_da + (row_float4_da) * K + col_float4_da);
    das[shared_flag][col_float4_da][row_float4_da] = da_reg[0];
    das[shared_flag][col_float4_da + 1][row_float4_da] = da_reg[1];
    das[shared_flag][col_float4_da + 2][row_float4_da] = da_reg[2];
    das[shared_flag][col_float4_da + 3][row_float4_da] = da_reg[3];
...
//         //}
//     }
    FETCH_FLOAT4(&dbs[shared_flag][row_float4_db][col_float4_db]) = FETCH_FLOAT4(block_start_address_db + (row_float4_db) * N + col_float4_db);
    __syncthreads();
...
//             for (int i = 0; i < BM; i += row_stride_da) {
//                 // if (i + row_float4_da < BM) {
//                 FETCH_FLOAT4(da_reg) = FETCH_FLOAT4(block_start_address_da + (i + row_float4_da) * K + col_float4_da);
//                 das[shared_flag][col_float4_da][i + row_float4_da] = da_reg[0];
//                 das[shared_flag][col_float4_da + 1][i + row_float4_da] = da_reg[1];
//                 das[shared_flag][col_float4_da + 2][i + row_float4_da] = da_reg[2];
//                 das[shared_flag][col_float4_da + 3][i + row_float4_da] = da_reg[3];
//                 //}
//             }
            FETCH_FLOAT4(da_reg) = FETCH_FLOAT4(block_start_address_da + (row_float4_da) * K + col_float4_da);
            das[shared_flag][col_float4_da][row_float4_da] = da_reg[0];
            das[shared_flag][col_float4_da + 1][row_float4_da] = da_reg[1];
            das[shared_flag][col_float4_da + 2][row_float4_da] = da_reg[2];
            das[shared_flag][col_float4_da + 3][row_float4_da] = da_reg[3];
