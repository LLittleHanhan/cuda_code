...
__global__ void flash_forward(void* output, const void* q, const void* k, const void* v, int len, float sm_scale) {
...
#pragma unroll
    ...
    const int bs_head_offset = base_id * kHeadDim * len;

    auto Q = make_tensor(make_gmem_ptr<half_t>((T*)q + bs_head_offset),
                         make_shape(len, Int<kHeadDim>{}),
...
#pragma unroll
    ...
    int n_block_max = cute::ceil_div(len, kBlockN);
#pragma unroll 1
...
int main() {
    ...
    half* q = new half[len * head_dim * bs * head_num];
    half* k = new half[len * head_dim * bs * head_num];
    half* v = new half[len * head_dim * bs * head_num];
    half* r = new half[len * head_dim * bs * head_num];

    Init(q, len, head_dim * bs * head_num, 0);
    Init(k, len, head_dim * bs * head_num, 1);
    Init(v, len, head_dim * bs * head_num, 2);

    half *dq, *dk, *dv, *dr;

    cudaMalloc(&dq, sizeof(half) * len * head_dim * bs * head_num);
    cudaMalloc(&dk, sizeof(half) * len * head_dim * bs * head_num);
    cudaMalloc(&dv, sizeof(half) * len * head_dim * bs * head_num);
    cudaMalloc(&dr, sizeof(half) * len * head_dim * bs * head_num);

    cudaMemcpy(dq, q, sizeof(half) * len * head_dim * bs * head_num, cudaMemcpyHostToDevice);
    cudaMemcpy(dk, k, sizeof(half) * len * head_dim * bs * head_num, cudaMemcpyHostToDevice);
    cudaMemcpy(dv, v, sizeof(half) * len * head_dim * bs * head_num, cudaMemcpyHostToDevice);

    config::FlashConfig<cute::half_t>
        config;
    int shm_size = config.kShmSize;
    auto partition_kernel = flash_forward<decltype(config)>;
    cudaFuncSetAttribute(partition_kernel,
    ...
    dim3 grid(len/64, bs * head_num);
    dim3 block(config.kThreadNum);

    printf("thread is %d\n", config.kThreadNum);
    partition_kernel<<<grid, block, shm_size>>>(
        (void*)dr, (const void*)dq,
        (const void*)dk, (const void*)dv ,len, sm_scale);
    cudaMemcpy(r, dr, sizeof(half) * len * head_dim * bs * head_num, cudaMemcpyDeviceToHost);
    // print(r, len, head_dim * bs * head_num);
}