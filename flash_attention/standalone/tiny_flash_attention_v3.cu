...
__global__ void flash_attention_kernel(int seq, half* q, half* k, half* v, half* r, float scale) {
    ...
    copy(g2s_copy_K, tKgK(_, _, _, 0), tKsK);
    cp_async_fence();
    copy(g2s_copy_V, tVgV(_, _, _, 0), tVsV);
    cp_async_fence();
...
#pragma unroll
        for (int i = 0; i < size<2>(tPrQ); i++) {
            copy(s2r_copy_K, tPsK(_, _, i), tPrK_copy(_, _, i));
            gemm(mma, tPrP, tPrQ(_, _, i), tPrK(_, _, i), tPrP);
        }

        // now can copy g->s next tile k
        if(n+1<seq/bc){
            copy(g2s_copy_K, tKgK(_, _, _, n + 1), tKsK);
            cp_async_fence();
        }
...
#pragma unroll
        ...
        __syncthreads();
#pragma unroll
        for (int i = 0; i < size<2>(tOrP_fp32); i++) {
            copy(s2r_copy_VT, tOsVT(_, _, i), tOrVT_copy(_, _, i));
#pragma unroll
            for (int j = 0; j < size<0>(tOrP); j++) {
                tOrP(j, 0, i) = __float2half(tOrP_fp32(j, 0, i));
            }
            gemm(mma, tOrO, tOrP(_, _, i), tOrVT(_, _, i), tOrO);
        }
        DEBUG_BLOCK(
            if (bid == 0 && tid == 0) {
                printf("tOrP: ");
        ...
        if(n + 1 < seq / bc){
             copy(g2s_copy_V, tVgV(_, _, _, n+1), tVsV);
             cp_async_fence();
        }
    }
    // normlize
...
void flash_attention() {
    ...
    // print(r, seq, dim * batch_size * head_num);
}
