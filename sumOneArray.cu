...
void sumOneArray(int tx, int m) {
    ...
    dim3 block(tx);
    dim3 grid(n / (tx * m));
    cout << "block num: "<<n / (tx * m) << "  thread num per block: " << tx << "  unroll: " << m ;
    reduce<<<grid, block>>>(ga, gres, m);
...
int main(int argc, char** argv) {
    ...
    cout << "max num blocks of per mutipro:" << prop.maxBlocksPerMultiProcessor << endl;
